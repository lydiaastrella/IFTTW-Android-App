...
public class SQLiteDatabaseHandler extends SQLiteOpenHelper {
    ...
    public boolean addRoutine(Routine routine, boolean active){
        if(isDataExist(routine.getFunctionality(), routine.getCondition(), routine.getAction())){
            return false;
        }else{
            long result;
    ...
    public boolean isDataExist(String functionality, String condition, String action){
        String selectQuery;
        selectQuery = "SELECT id FROM ActiveRoutine WHERE "
    ...
    public boolean deleteData(String functionality, String condition, String action, boolean active)
    {
        SQLiteDatabase db = getWritableDatabase();
        if (active){
            return db.delete(TABLE_NAME_ACTIVE, KEY_FUNCTIONALITY + "= \"" + functionality + "\" AND " +KEY_CONDITION + "= \"" +condition+ "\" AND " +KEY_ACTION + "= \"" +action + "\"", null) > 0;
        }else{
